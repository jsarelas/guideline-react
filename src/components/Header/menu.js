...
class Menu extends Component {
    ...
    componentDidMount() {
        ...
        document.getElementById("gl-overlay").addEventListener('mousedown', this.toggleMenu);
        document.getElementById("gl-overlay").addEventListener('touchstart', this.toggleMenu);
    }
        ...
        return (
                <div id="sidenav" className={(this.state.sideNav)}>
                    ...
                    <li><Link to="/contact" onClick={this.itemSelected}><i className="user icon nav-icon"></i>Contact</Link></li>
                        ...
                        <li className="sidenav-divider"></li>
                        <li><div>RabbitMQ Samples</div><div>(coming soon)</div></li>
                    </ul>
                </div>   
        );
    }
